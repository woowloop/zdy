...
//        }
		...
		for(int i=0; i<m_users.Count; i++)
		{
			...
			for (int j = i+1; j < m_users.Count; j++)
			{
				...
				if (curUser.m_comX < -900 && curUser.m_comX > 900)
				{
					continue;
				}
				
				if (Mathf.Abs(curUser.m_comX) < Mathf.Abs(bestUser.m_comX) || bestUser.m_comX==0) {

                    if (Mathf.Abs(curUser.m_priority) < Mathf.Abs(bestUser.m_priority) && Mathf.Abs(curUser.m_priority) > 1000 && Mathf.Abs(curUser.m_priority) < 3500)// && curUser.m_priority < 2500
                    {
						bestUser = curUser;
						bestIndex = j;
					}
				...
				if (bestUser.m_comX < -900 || bestUser.m_comX > 900 || (bestUser.m_comX == 0)) {
					bestUser = curUser;
					bestIndex = j;
				}
            ...
            //Debug.Log(bestUser.m_priority + "bestUser.m_priority");
    ...
    protected virtual void UpdateUserPriority(NIPlayerCandidateObject user)
    {
		...
		userCom.m_comX = com.x;
		userCom.m_comY = com.y;
